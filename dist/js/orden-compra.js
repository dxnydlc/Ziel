...
(function($){
	...
	$(document).ready(function(){
		...
		$(document).delegate('.anularPedido', 'click', function(event) {
			...
			alertify.confirm('Confirme anular Orden de Compra',function(e){
				if(e){
		...
		$(document).delegate('.copiarPedido', 'click', function(event) {
			/**/
			var _idPedido = $(this).attr('href');
			alertify.confirm('Confirme copiar Orden de compra #'+_idPedido,function(e){
				if(e){
					$('#CopiarModal').modal('show');
		...
		$(document).delegate('.goPedido', 'click', function(event) {
			...
			//
			alertify.confirm('Confirme Ingresar Orden de Compra',function(e){
				if(e){
					$.post( _servicio , { f:'goParteEntrada', idp:_idPedidoX } , function(data, textStatus, xhr) {
						if( data.idPE > 0 ){
		...
		$('#btnGoFacturar').click(function(event) {
			...
			$.post( _servicio , { f:_filtro, idp:_idPedidoX, 'TipoDoc':_docGenerate, serie:_serieUsar} , function(data, textStatus, xhr) {
				if( data.idVenta > 0 ){
					document.location.href = 'nueva-venta.php?id='+data.idVenta;
				}
			},'json');
			/**/
			event.preventDefault();
