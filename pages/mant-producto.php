...
$data_prods = $OBJ->get_productos_lista( '' , $limite );
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Mantenimiento Productos</h1>
                    <p>Productos que se mostrarán en el sistema</p>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                    ...
                    <nav class="navbar navbar-default">
                        <div class="container-fluid">
                            ...
                            <div class="navbar-header">
                                <a id="NuevoItem" href="editar-producto.php" class="btn btn-primary btn-outline navbar-btn " >
                                        ...
                                        <table id="ItemsResponse" class="table" >
                                            ...
                                            if( is_array( $data_prods['data'] ) ){
                                                foreach ( $data_prods['data'] as $key => $rs ) {
                                                    ...
                                                    if( $rs["chr_Destacado"] == 1 ){
                                                        $ico = '<span class="fa fa-bookmark " ></span>';
                                                    }else{
                                                        $ico = '';
                                                    }

                                                    $arr_clases = array();
                                                    $arr_clases = $rs["clases"];
                                                    $txtClases  = '';
                                                    if( is_array( $arr_clases ) ){
                                                        $c = array();
                                                        foreach ($arr_clases as $key => $rc) {
                                                            array_push( $c , $rc->name );
                                                        }
                                                        $txtClases = implode(', ', $c );
                                                    }
                                                    echo '<tr>';
                                                        echo '<td>'.$rs["int_IdProducto"].'</td>';
                                                        echo '<td>'.$ico.'</td>';
                                                        echo '<td>';
                                                            echo '<a href="editar-producto.php?idp='.$rs["int_IdProducto"].'" ><span class="fa fa-barcode" ></span> '.$rs["producto"].'</a>';
                                                        echo '</td>';
                                                        $arPrecio = $rs["arPrecio"];
                                                        $textoAr = array();
                                                        $textoAr = split(',', $arPrecio );
                                                        echo '<td class="text-right" >'.$textoAr[0].'</td>';
                                                        echo '<td>'.$txtClases.'</td>';
                                                        echo '<td>'.$rs["generico"].'</td>';
...
<?php include_once('popup1.php'); ?>
    ...
    <script src="../dist/js/mant_producto.js<?php echo $rand; ?>"></script>
